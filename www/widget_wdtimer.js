...
function depends_wdtimer (){   
	...
	if (userCSS.length)
		...
		$('head').append('<link rel="stylesheet" href="'+ ftui.config.basedir + 'css/fhem-tablet-ui-wdtimer.css" type="text/css" />');
...
var Modul_wdtimer = function () {
	...
	function wdtimer_showDialog(elem,device) { //Erstellen des Dialogs und öffnen des Dialogs
		...
		if( elem.data('codemirror') && typeof AddCodeMirror == 'function' ) {
			var cmObj;
			AddCodeMirror(elem.find('.wdtimer_code.command')[0], function(pcm) {
				...
				cmObj.attr({'style':cmObj.attr('style')+'; text-align:left; resize:none; visibility:visible; height: inherit; width:inherit !important;'});
				cmObj.attr({'type':'text', 'name':'wdtimer_text_code'});
			});
		} else {
			$(elem.find('.wdtimer_code.command')).append("<textarea class='wdtimer_text command inline "+
					config[2][8]+" "+config[2][9]+
					"' type='text' style='white-space:pre; resize:none; visibility:visible; height:inherit; width:inherit !important;' name='wdtimer_text_code'></textarea>");
		...
		var wdtimer_dialog = $( ".wdtimer_dialog" ).dialog(wdtimer_dialogConfig);
		...
		wdtimer_setDateTimePicker($('.wdtimer_'+device.replace(/\./g,'\\.')), device,config[2][9],config[2][11]); //DateTimePicker Plugin zuweisen
		...
		$('.wdtimer_'+device.replace(/\./g,'\\.')).find('[name^="wdtimer_text"]').trigger('input');
		$('.wdtimer_'+device.replace(/\./g,'\\.')).find('[name^="wdtimer_time"]').trigger('input');
	...
	function wdtimer_closeDialog(elem,device) {
		var wdtimer_dialog = $( ".wdtimer_dialog" );
		wdtimer_dialog.dialog( "close" );
		$('.wdtimer_'+device.replace(/\./g,'\\.')).remove();                    
		$('.wdtimer_datetimepicker_'+device.replace(/\./g,'\\.')).each(function(){ $(this).remove(); });
		elem.children('.wdtimer_dialog').remove();                  
	...
	function wdtimer_addProfile(elem, device) {
		...
		$('.wdtimer_'+device.replace(/\./g,'\\.')).find('.wdtimer_profilelist').append(profile_row);
		$('.wdtimer_'+device.replace(/\./g,'\\.')).find('[name="wdtimer_timedd"]').on('change',function(evt) {wdtimer_showhideoptions(evt);});
		$('.wdtimer_'+device.replace(/\./g,'\\.')).find('[name^="wdtimer_text"]').on('input',function(evt) {wdtimer_changewidth(evt,5,100);});
		$('.wdtimer_'+device.replace(/\./g,'\\.')).find('[name^="wdtimer_time"]').on('input',function(evt) {wdtimer_changewidth(evt,5,100);});

		wdtimer_setDeleteAction($('.wdtimer_'+device.replace(/\./g,'\\.')), config[2][0]); //Löschen-Schalter Aktion zuweisen      
		wdtimer_setDateTimePicker($('.wdtimer_'+device.replace(/\./g,'\\.')), config[2][0],config[2][9],config[2][11]); //DateTimePicker Plugin zuweisen zuweisen              
   ...
   function wdtimer_updateProfiles(device) {
	...
	function init() {
		...
		this.elements.each(function(index) {
            ...
            elem.initData('width',    $(this).data('width') || '510');
